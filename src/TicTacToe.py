...
class TicTacToe:
    def __init__(self, smartMovePlayer1=0, playerSQN=None):
        ...
        assert (
            0 <= smartMovePlayer1 <= 1
        ), "Probability of Smart Move must lie between 0 and 1"
    ...
    def print_board(self):
        ...
        for i in range(3):
            print(
                f" {board_symbols[3 * i]} | {board_symbols[3 * i + 1]} | {board_symbols[3 * i + 2]} "
            )
            if i < 2:
                print("---+---+---")
        print()
